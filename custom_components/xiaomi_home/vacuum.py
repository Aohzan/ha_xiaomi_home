...
class Vacuum(MIoTServiceEntity, StateVacuumEntity):
    ...
    async def async_send_command(
        ...
        **kwargs: Any,
        ...
        Supported commands:
          clean_room  – params: room_ids (list[str]), fan_level (int),
                        water_level (int), sweep_type (int), sweep_count (int)
          clean_zone  – params: zone_ids (list[str]), fan_level (int),
                        water_level (int), sweep_type (int), sweep_count (int)
        """
        if params is None:
            params = {}
        ...
        if command == 'clean_room':
            ...
            if self._action_start_zone_sweep is None:
                _LOGGER.error(
                    'send_command clean_zone: start-zone-sweep action '
                    'not available for %s', self.entity_id)
                return
            zone_ids = params.get('zone_ids', [])
            ids_json = json.dumps(
                [str(z) for z in zone_ids] if zone_ids else [])
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return extra state attributes for advanced vacuum properties."""
        attrs: dict[str, Any] = {}

        if self._prop_sweep_type is not None and self._sweep_type_map:
            val = self.get_prop_value(prop=self._prop_sweep_type)
            if val is not None:
                attrs['sweep_type'] = self.get_map_value(
                    map_=self._sweep_type_map, key=val)

        if self._prop_water_level is not None and self._water_level_map:
            val = self.get_prop_value(prop=self._prop_water_level)
            if val is not None:
                attrs['water_level'] = self.get_map_value(
                    map_=self._water_level_map, key=val)

        if self._prop_sweep_count is not None and self._sweep_count_map:
            val = self.get_prop_value(prop=self._prop_sweep_count)
            if val is not None:
                attrs['sweep_count'] = self.get_map_value(
                    map_=self._sweep_count_map, key=val)

        # Expose available room/zone sweep capability as attributes
        if self._action_start_room_sweep is not None:
            attrs['supports_room_clean'] = True
        if self._action_start_zone_sweep is not None:
            attrs['supports_zone_clean'] = True
